...
if __name__ == '__main__':
    ...
    parser.add_argument('--unlearned_model_config', type=str,)
    parser.add_argument('--corrupted_dataset', type=str,)
 
    args = parser.parse_args()

    base_model = LlamaForCausalLM.from_pretrained(BASE_MODEL_PATH)
    model = PeftModel.from_pretrained(base_model, f'{UNLEARNED_MODELS_PATH}/{args.unlearned_model_config}') 
    tokenizer = AutoTokenizer.from_pretrained(f'{UNLEARNED_MODELS_PATH}/{args.unlearned_model_config}', padding_side='left')


    print('Task Vector Evaluation [Good to Go!]\n========================\n')
    ...
    dir_to_save = f'outputs/unlearned/{args.corrupted_dataset}/task_vector_{args.unlearned_model_config}'
